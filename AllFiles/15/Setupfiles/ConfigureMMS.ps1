...
Write-Host
Write-Host "Verifying the SharePoint Managed Metadata Service is running..." -ForegroundColor White
$MMSSvcInstance = Get-SPServiceInstance | Where-Object { $_.TypeName -eq "Managed Metadata Web Service" }
if ($MMSSvcInstance.Status -ne "Online") {
  Write-Host "SharePoint Managed Metadata Service is not running..." -ForegroundColor Gray 
  Write-Host "Starting SharePoint Managed Metadata Service..." -ForegroundColor Gray 
  $silence = Start-SPServiceInstance -Identity $MMSSvcInstance
  Write-Host "SharePoint Managed Metadata Service started" -ForegroundColor Gray 
} Else {
  Write-Host "SharePoint Managed Metadata Service is aleady running" -ForegroundColor Gray 
}
...
Write-Host
Write-Host "Creating Managed Metadata Service Application..." -ForegroundColor White
Write-Host "Creating Managed Metadata Service Application..." -ForegroundColor Gray 
$mms = New-SPMetadataServiceApplication –ApplicationPool $appPoolServiceApps –Name "Contoso Managed Metadata" –DatabaseName "ContosoMMS_DB"
Write-Host "Managed Metadata Service Application created" -ForegroundColor Gray 
Write-Host "Creating Managed Metadata Service Application proxy..." -ForegroundColor Gray 
$mmsProxy = New-SPMetadataServiceApplicationProxy –ServiceApplication $mms -Name "Contoso Managed Metadata" -DefaultProxyGroup
$mmsProxy.Properties.IsDefaultSiteCollectionTaxonomy = $true
$mmsProxy.Update()
Write-Host "Managed Metadata Service Application proxy created" -ForegroundColor Gray 

Write-Host
Write-Host "Finished configuring the Managed Metadata Service." -ForegroundColor Green
Write-Host
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUJ3EWuEO9NFWDn3TgMbQxkY8C
# DrOgghWCMIIEwzCCA6ugAwIBAgITMwAAADQkMUDJoMF5jQAAAAAANDANBgkqhkiG
...
# HAYKKwYBBAGCNwIBCzEOMAwGCisGAQQBgjcCARUwIwYJKoZIhvcNAQkEMRYEFOFO
# +yI7gLk6EQabqIe8fhAnGivSMF4GCisGAQQBgjcCAQwxUDBOoCaAJABNAGkAYwBy
# AG8AcwBvAGYAdAAgAEwAZQBhAHIAbgBpAG4AZ6EkgCJodHRwOi8vd3d3Lm1pY3Jv
# c29mdC5jb20vbGVhcm5pbmcgMA0GCSqGSIb3DQEBAQUABIIBACIxfiXxySZ/er+t
# Bddsqo4ywaqkiwN6sHqUW/e7UjlzOodX+Z3RitaVCVOd4uOSIYUaUwldKoKIdr+f
# 9k9ZCB81NiaaHSS9JO3aliMqZIYwuU6T3iWb2aeGBvNTa98DxS1r9m1PLJwLxk5E
# ot4qiJPrdZHvtgTdeL5t9gpi4vJAE/RdEmcuNkNYbPo06miXhntEFKvzitWOtAlL
# ++hzsJzr5U0a5yORJy32BABLbtKtax09z1xsupZXZl9uDglxlF6s/iHc3kQBVoUf
# L+WOGZD95CIvQvcPB95iPJ6frwrF3Wq8VU3UkmINGQwD/09JJ6jht4+lChPZKrxj
# oq/0cmmhggIoMIICJAYJKoZIhvcNAQkGMYICFTCCAhECAQEwgY4wdzELMAkGA1UE
...
# MDExODAxMDFaMCMGCSqGSIb3DQEJBDEWBBTfxleQWV7tepmLE5oAHfpi2WNkzDAN
# BgkqhkiG9w0BAQUFAASCAQB2b7zOvpoSQa7V5Mxw8R9MK8NGI70Fb0JgBnG2FBtH
# rBLGTCWSGKk9HKgPXfnzrnMoUoKI2yS9ui+/lqLI/ulU6Su3MHJ8yFPH0B7/1WvT
# +3I3Ck3Gfy3ekrf9pWjbFBC1nK+2CN6Jp8vRdUzYIy+WcDAYl77p9QEYA3nIH9OJ
# mm8y3q3uDHO+Q8U58OQg3jRSHiN+1Rc6Snoulf+CjZxaG1Qg1q8WFi9/bTNXHgxY
# bT8SKQMEMGT8qfVq4VMk2xAT5gw+6GNsSR0js/6Hg8lpNjXHH5ak4KmjRrn41mXM
# CDn8yyrtSxMa4n2KCLXdp27db4jzdHJmOC7B/P8nwO3w
