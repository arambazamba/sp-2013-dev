Write-Host
Write-Host "Configuring Workflow Manager and Service Bus..." -ForegroundColor White
Write-Host
...
# Create new SB Farm
$SBCertificateAutoGenerationKey = $password;


New-SBFarm -SBFarmDBConnectionString 'Data Source=LONDON.Contoso.net;Initial Catalog=SbManagementDB;Integrated Security=True;Encrypt=False' -InternalPortRangeStart 9000 -TcpPort 9354 -MessageBrokerPort 9356 -RunAsAccount 'Administrator@contoso.net' -AdminGroup 'BUILTIN\Administrators' -GatewayDBConnectionString 'Data Source=LONDON.Contoso.net;Initial Catalog=SbGatewayDatabase;Integrated Security=True;Encrypt=False' -CertificateAutoGenerationKey $SBCertificateAutoGenerationKey -MessageContainerDBConnectionString 'Data Source=LONDON.Contoso.net;Initial Catalog=SBMessageContainer01;Integrated Security=True;Encrypt=False' -Verbose;

# To be run in Workflow Manager PowerShell console that has both Workflow Manager and Service Bus installed.

# Create new WF Farm
$WFCertAutoGenerationKey = $password;


New-WFFarm -WFFarmDBConnectionString 'Data Source=LONDON.Contoso.net;Initial Catalog=WFManagementDB;Integrated Security=True;Encrypt=False' -RunAsAccount 'Administrator@contoso.net' -AdminGroup 'BUILTIN\Administrators' -HttpsPort 12290 -HttpPort 12291 -InstanceDBConnectionString 'Data Source=LONDON.Contoso.net;Initial Catalog=WFInstanceManagementDB;Integrated Security=True;Encrypt=False' -ResourceDBConnectionString 'Data Source=LONDON.Contoso.net;Initial Catalog=WFResourceManagementDB;Integrated Security=True;Encrypt=False' -CertificateAutoGenerationKey $WFCertAutoGenerationKey -Verbose;

# Add SB Host
$SBRunAsPassword = $password;


Add-SBHost -SBFarmDBConnectionString 'Data Source=LONDON.Contoso.net;Initial Catalog=SbManagementDB;Integrated Security=True;Encrypt=False' -RunAsPassword $SBRunAsPassword -EnableFirewallRules $true -CertificateAutoGenerationKey $SBCertificateAutoGenerationKey -Verbose;

Try
{
    # Create new SB Namespace
    New-SBNamespace -Name 'WorkflowDefaultNamespace' -AddressingScheme 'Path' -ManageUsers 'Administrator@contoso.net' -Verbose;
...
# Add WF Host
$WFRunAsPassword = $password;


Add-WFHost -WFFarmDBConnectionString 'Data Source=LONDON.Contoso.net;Initial Catalog=WFManagementDB;Integrated Security=True;Encrypt=False' -RunAsPassword $WFRunAsPassword -EnableFirewallRules $true -SBClientConfiguration $SBClientConfiguration -EnableHttpPort  -CertificateAutoGenerationKey $WFCertAutoGenerationKey -Verbose;

Write-Host "Finished configuring Workflow Manager and Service Bus" -ForegroundColor Green
Write-Host
...
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQ